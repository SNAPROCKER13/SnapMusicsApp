    ...
    return(
        <div className="w-full bg-white flex flex-row border-2 border-black items-center justify-between px-12 h-24">
            ...
            <div className="flex flex-row justify-center items-center">
                {location.pathname !== '/profile' ? <div onClick={()=>{
                    pause()
                    navigate('/profile')
                }} className="bg-white rounded-full my-14 w-16 h-16 flex justify-center items-center border-2 border-black mx-6">Profile</div> : <button onClick={() => navigate(-1)} className="border-2 border-black rounded-lg h-max w-max text-black mx-6">Back</button>}
                <button onClick={() => {
